...
class BasicStem(nn.Sequential):
    ...
    def __init__(self, width_deduction_ratio = 1.0) -> None:
        super().__init__(
            nn.Conv3d(3, int(64/width_deduction_ratio), kernel_size=(3, 7, 7), stride=(1, 2, 2), padding=(1, 3, 3), bias=False),
            nn.BatchNorm3d(int(64/width_deduction_ratio)),
            nn.ReLU(inplace=True),
        )
...
class VideoResNet(nn.Module):
    def __init__(
        ...
        if stem_deduct:
            self.inplanes = int(64/width_deduction_ratio)
            self.stem = stem(width_deduction_ratio = width_deduction_ratio)
        else:
            self.inplanes = 64
            self.stem = stem()
        
        self.layer1 = self._make_layer(block, conv_makers[0], int(64/width_deduction_ratio), layers[0], stride=1)
        self.layer2 = self._make_layer(block, conv_makers[1], int(128/width_deduction_ratio), layers[1], stride=2)
        self.layer3 = self._make_layer(block, conv_makers[2], int(256/width_deduction_ratio), layers[2], stride=2)
        self.layer4 = self._make_layer(block, conv_makers[3], int(512/width_deduction_ratio), layers[3], stride=2)
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.fc = nn.Linear(int(512/width_deduction_ratio) * block.expansion, num_classes)
...
def r3d_18(*, progress: bool = True, **kwargs: Any) -> VideoResNet:
...
def mc3_18(*, progress: bool = True, **kwargs: Any) -> VideoResNet:
