...
class VICCLRSDTD3P(nn.Module): 
    def __init__(
      ...
      if proj_layer > 0:
          ...
          self.projector_sd = create_mlp_fn(feature_size, projection_size, projection_hidden_size, proj_layer)
          self.projector_td = create_mlp_fn(feature_size, projection_size, projection_hidden_size, proj_layer)
      else:
          self.projector1 = nn.Identity()
          self.projector_sd = nn.Identity()
          self.projector_td = nn.Identity()
      # different projector followed by two different encoder*(same architecture but with different initialization)
      # for encoder 2(sees spatial and temporal diff), 
      # projector_sd projects the spatial-related feature, 
      # projector_td projects the temporal-related feature
    ...
    def forward(
        ...
        gt_z_all_sd = self.projector_sd(hidden_sd)
        gt_z_all_td = self.projector_td(hidden_td)
