...
def get_data_ucf_rand_derivative_average(transform_consistent=None,
                transform_inconsistent=None,
                mode='train', 
                seq_len=4, 
                num_seq=3, 
                ...
                random=False,
                prob_derivative = 0.5,
                prob_average_frame = 0.5,
                inter_len=0, # num of frames (after downsampling) between two clips
                fraction=1.0
                ):
    print('Loading data for "%s" ...' % mode)
    dataset = UCF101RandDerivativeAverage(mode=mode,
                        transform_consistent=transform_consistent,
                        transform_inconsistent=transform_inconsistent,
                        seq_len=seq_len,
                        num_seq=num_seq,
                        ...
                        frame_root=frame_root,
                        random=random,
                        prob_derivative = prob_derivative,
                        prob_average_frame = prob_average_frame,
                        inter_len=inter_len
                        )
    if not ddp:
        sampler = data.RandomSampler(dataset)
...
def get_data_ucf_derivative(transform_consistent=None,
                transform_inconsistent=None,
                mode='train', 
                seq_len=4, 
                num_seq=3, 
    ...
    print('Loading data for "%s" ...' % mode)
    dataset = UCF101Derivative(mode=mode,
                        transform_consistent=transform_consistent,
                        transform_inconsistent=transform_inconsistent,
                        seq_len=seq_len,
                        num_seq=num_seq,
...
class UCF101RandDerivativeAverage(data.Dataset):
    def __init__(self,
                mode='train',
                transform_consistent=None,
                transform_inconsistent=None,
        ...
        self.frame_root = frame_root
        self.random = random
        self.prob_derivative = prob_derivative
        self.prob_average_frame = prob_average_frame
        self.inter_len = inter_len
        self.total_len = ((self.seq_len + self.inter_len)*self.num_seq - self.inter_len)*self.downsample
        ...
        if prob_derivative > 1 or prob_derivative < 0:
            raise ValueError('Invalid value of probability of taking derivative')
        
        if prob_average_frame > 1 or prob_average_frame < 0:
            raise ValueError('Invalid value of probability of averaging frames')
    ...
    def __getitem__(self, index):
        # return seq_rand_derivative, seq_rand_average, label(optional)
        # with prob = args.prob_derivative, seq_derivative being the derivative of video frames
        # with prob = (1 - args.prob_derivative, seq_derivative being the original video frames but drop the last one
        # with prob = args.prob_average_frame, seq_rand_average being the average across frames
        # with prob = (1 - args.prob_average_frame), seq_rand_average being the original video frame but drop the last one
        vpath, vlen, aid = self.video_info.iloc[index]
        items = self.idx_sampler(vlen, vpath)
        if items is None: print(vpath) 
        ...
        if self.return_label:
            label = torch.LongTensor([aid])
            return seq_rand_derivative, seq_rand_average, label
        return seq_rand_derivative, seq_rand_average
...
class UCF101Derivative(data.Dataset):
    def __init__(self,
                mode='train',
                transform_consistent=None,
                transform_inconsistent=None,
    ...
    def __getitem__(self, index):
        ...
        if self.return_label:
            label = torch.LongTensor([aid])
            return seq[:, :, :-1, :, :], seq_derivative, label 
            # To make the shape consistent, drop the last frame of seq
        else:
            return seq[:, :, :-1, :, :], seq_derivative 
            # to make the shape consistent, drop the last frame of seq
        
        # __getitem__ returns [seq, seq_derivative, label(optional)]
        # the first element of the tuple is seq and the second one is derivative,
        # We feed the seq_derivative to encoder 1 when epoch index(start with 0) is even
        # and feed seq to encoder 1 when epoch index is odd;
        # We always feed seq to encoder2 
    def __len__(self):
        return len(self.video_info)
