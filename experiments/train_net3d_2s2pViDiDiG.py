...
def train_one_epoch(args, model, train_loader, optimizer, epoch, gpu=None, scaler=None, train=True, diff=False, mix=False, mix2=False):
    ...
    for step, data in enumerate(train_loader, start=epoch * len(train_loader)):
        ...
        with torch.cuda.amp.autocast():
            loss = model(video, rand_diff, epoch)
            if train:
...
def main():
    ...
    ckpt_folder='/data/checkpoints_yehengz/2s2p_ViDiDi_as_guidance/%s%s_%s_%s/sym%s_bs%s_lr%s_wd%s_ds%s_sl%s_nw_rand%s_feature_size%s_projection%s_proj_hidden%s_epochs%s_seed%s_width_deduc_ratio%s_stem_deduct%s' \
