...
# torchrun --standalone --nnodes=1 --nproc_per_node=8 experiments/mytune_net3d_coeffs.py --epochs 400 --batch_size 64 --sym_loss --base_lr 4.8 --projection 2048 --proj_hidden 2048 --pred_layer 0 --proj_layer 3 --cov_l 0.04 --std_l 1.0 --spa_l 0.0
...
parser.add_argument('--wd', default=1e-6, type=float, help='weight decay') # default learning rate is 1e-6
...
parser.add_argument('--mse_l', default=[100, 10, 0.1], type=list) # default value used is 1
parser.add_argument('--std_l', default=[100, 10, 0.1], type=list) # default value used is 1
parser.add_argument('--cov_l', default=[4, 0.4, 0.004], type=list) # default value used is 0.04
# parser.add_argument('--mse_l', default=[100,0.01], type=list) # default value used is 1
# parser.add_argument('--std_l', default=[100, 0.01], type=list) # default value used is 1
# parser.add_argument('--cov_l', default=[40], type=list) # default value used is 0.04
...
def main():
    ...
    if args.which_experiment == 1:
        ckpt_folder='/home/yehengz/Func-Spec/hyperparams-tuning/coeffs/2S/%s%s_%s_%s/sym%s_ds%s_sl%s_nw_rand%s' \
            % (dataname, args.fraction, ind_name, model_name, args.sym_loss, args.downsample, args.seq_len, args.random)
    elif args.which_experiment == 2:
        ckpt_folder='/home/yehengz/Func-Spec/hyperparams-tuning/coeffs/NWS/%s%s_%s_%s/sym%s_ds%s_sl%s_nw_rand%s_fixed_pair%s' \
            % (dataname, args.fraction, ind_name, model_name, args.sym_loss, args.downsample, args.seq_len, args.random, args.which_fixed_pair)
    else:
        ckpt_folder='/home/yehengz/Func-Spec/hyperparams-tuning/coeffs/fixed_pair/%s%s_%s_%s/sym%s_ds%s_sl%s_nw_rand%s_fixed_pair%s' \
        % (dataname, args.fraction, ind_name, model_name, args.sym_loss, args.downsample, args.seq_len, args.random, args.which_fixed_pair)
    ...
    all_best_result = []
    hyper_params_choice = []
    for mse_l in args.mse_l:
        for std_l in args.std_l:
            for cov_l in args.cov_l:
                hp_choice = 'MSE coeff %s, STD coeff %s, COV coeff %s' % (mse_l, std_l, cov_l)
                hyper_params_choice.append(hp_choice)
                print('Hyperparameters tuning for mse_l %s, std_l %s, cov_l %s starts' %
                            (mse_l, std_l, cov_l))
                logging.info('==========Hyperparameters tuning for mse_l %s, std_l %s, cov_l %s starts==========' %
                            (mse_l, std_l, cov_l))
            ...
            if args.rank == 0:
                ...
                logging.info('==========Hyperparameters tuning for mse_l %s, std_l %s, cov_l %s complete==========' %
                             (mse_l, std_l, cov_l))
