...
from vicclr2s2prdramix import VICCLR2S2PRDRAMIX
...
parser.add_argument('--AdamW', action = 'store_true') # default is false, meaning we will use LARS optimizer. Remember to adjust learning rate and weight decay
...
parser.add_argument('--prob_derivative', default = 1.0, type = float) # d/dt to all data
parser.add_argument('--prob_average', default = 1.0, type = float) # avg to all data
...
def train_one_epoch(args, model, train_loader, optimizer, epoch, gpu=None, scaler=None, train=True, diff=False, mix=False, mix2=False):
    ...
    for step, data in enumerate(train_loader, start=epoch * len(train_loader)):
        ...
        # N = 2 by default
        video_dt, video_avg, label = data # B, N, C, T, H, W
        label = label.to(gpu)
        video_dt = video_dt.to(gpu)
        video_avg = video_avg.to(gpu)
...
def main():
    ...
    model_select = VICCLR2S2PRDRAMIX
    ...
    # else:
    #     which_optimizer = "LARS"

    ckpt_folder='/data/checkpoints_yehengz/2s2p_rdra_mixed/%s%s_%s_%s/sym%s_bs%s_lr%s_wd%s_ds%s_sl%s_nw_rand%s_feature_size%s_projection%s_proj_hidden%s_epochs%s_seed%s_operation%s_prob_derivative%s_prob_average%s_width_deduc_ratio%s_stem_deduct%s' \
...
# torchrun --standalone --nnodes=1 --nproc_per_node=8 experiments/train_net3d_2s2prdramix.py --sym_loss --infonce --epochs 400 --seed 233 --feature_size 363 --projection 1452 --proj_hidden 1452 --width_deduction_ratio 1.41
