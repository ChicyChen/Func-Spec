...
def main():
    ...
    if not args.random and not args.kinetics:
        resnet1.load_state_dict(torch.load(ckpt_path1)) # load model
        resnet2.load_state_dict(torch.load(ckpt_path2)) # load model
    ...
    if args.random:
        ...
        if args.weighted_knn:
            test(resnet1,resnet2, train_loader, test_loader, args, args.concat)
        else:
